...
function qwenUsage(body) {
  ...
  if (!Number.isInteger(input) || input < 0 || input > 3) throw new Error("missing or invalid Qwen input_image_count; settlement requires reconciliation");
  if (!Number.isInteger(output) || output < 1 || output > 6) throw new Error("missing or invalid Qwen output_image_count; settlement requires reconciliation");
  if (!["qima_input_1k", "qima_input_2k"].includes(usage.input_image_type) || !["qima_output_1k", "qima_output_2k"].includes(usage.output_image_type)) throw new Error("missing or invalid Qwen image billing type; settlement requires reconciliation");
  const images = imageContent(body).filter(function(p) {return p.image;});
  if (images.length !== output) throw new Error("Qwen output count disagrees with image results; settlement requires reconciliation");
