...
// Private extension of QuantumNous Alibaba 1.3.0; not an official release.
const QWEN_IMAGE_MODELS = ["qwen-image-3.0", "qwen-image-3.0-pro"];
const QWEN_USAGE_SCHEMA = {
  qima_input_1k: { type: "number", unit: "count", description: {en: "1K input image unit price", zh: "1K输入图片单价"} },
  qima_input_2k: { type: "number", unit: "count", description: {en: "2K input image unit price", zh: "2K输入图片单价"} },
  qima_output_1k: { type: "number", unit: "count", description: {en: "1K output image unit price", zh: "1K输出图片单价"} },
  qima_output_2k: { type: "number", unit: "count", description: {en: "2K output image unit price", zh: "2K输出图片单价"} },
};

export const meta = {
  apiVersion: 1,
  key: "alibaba",
  name: "Alibaba Bailian · Qwen extension",
  icon: "Bailian.Color",
  description: {
    en: "Private Qwen Image 3.0 extension of the QuantumNous Alibaba image and video plugin",
    zh: "基于QuantumNous官方插件的私有扩展：万相图片视频与千问图片3.0",
  },
  ...
  usageProfiles: [
    { models: QWEN_IMAGE_MODELS, schema: QWEN_USAGE_SCHEMA },
    {
  ...
  routes: [
    { method: "POST", path: "/ali/compatible-mode/v1/images/generations", type: "submit", models: QWEN_IMAGE_MODELS, decode: "createQwenCompatible", render: "qwenImageCreated" },
    { method: "POST", path: "/ali/api/v1/services/aigc/multimodal-generation/generation", type: "submit", models: ["qwen-image-3.0", "qwen-image-3.0-pro", "wan2.7-image-pro", "wan2.7-image", "wan2.6-image", "wan2.6-t2i"], decode: "createImageTask", render: "imageCreated" },
    { method: "POST", path: "/ali/api/v1/services/aigc/image-generation/generation", type: "submit", models: ["qwen-image-3.0", "qwen-image-3.0-pro", "wan2.7-image-pro", "wan2.7-image", "wan2.6-image", "wan2.6-t2i"], decode: "createImageTask", render: "taskCreated" },
...
function convertImage(ctx) {
  if (isQwen(ctx)) return convertQwen(ctx);
...
function isQwen(ctx) {
  if (!ctx) return false;
  return QWEN_IMAGE_MODELS.includes(ctx.upstreamModel || ctx.model || (ctx.requestBody || {}).model);
}

function emptyQwenUsage() {
  return {qima_input_1k: 0, qima_input_2k: 0, qima_output_1k: 0, qima_output_2k: 0};
}

// Runtime-portable validation: no Node, Buffer, URL, or network dependency.
function qwenHTTPImage(value) {
  if (typeof value !== "string" || /[\s\u0000-\u001f\u007f\\]/.test(value)) return false;
  const match = /^https?:\/\/([^/?#]+)(?:[/?#]|$)/i.exec(value);
  if (!match || !/^(?:\[[0-9a-f:.]+\]|[a-z0-9][a-z0-9.-]*)(?::[0-9]{1,5})?$/i.test(match[1]) || /[<>"']/.test(value)) return false;
  const port = /:(\d+)$/.exec(match[1]);
  return !port || (Number(port[1]) > 0 && Number(port[1]) <= 65535);
}

function validateQwenImage(value) {
  if (qwenHTTPImage(value)) return;
  if (typeof value === "string" && /^data:image\//i.test(value)) {
    const match = /^data:image\/[a-z0-9.+-]+;base64,([A-Za-z0-9+/]*={0,2})$/i.exec(value);
    if (match && match[1].length > 0 && match[1].length % 4 === 0) return;
    throw new Error("[QWEN_INVALID_IMAGE] 图片 Base64 格式无效；请使用完整的 data:image/...;base64, 数据，不要自动重试原请求");
  }
  throw new Error("[QWEN_INVALID_IMAGE] image must be an HTTP URL or Base64 data URL；不允许空地址、空白字符或 URL 内嵌账号密码");
}

function qwenRequestId(body) {
  const id = body && body.request_id;
  return typeof id === "string" && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id) ? id : "";
}

function qwenDiagnostic(body, message) {
  const id = qwenRequestId(body);
  return message + (id ? " (request_id=" + id + ")" : "");
}

function qwenUpstreamError(body) {
  const output = body.output || {};
  const raw = body.code || output.code || "upstream_error";
  const code = typeof raw === "string" && /^[A-Za-z][A-Za-z0-9_.]{0,63}$/.test(raw) ? raw : "upstream_error";
  let hint = "原厂返回错误，请按请求 ID 核查；不要盲目重新生成";
  if (/throttl|ratelimit|toomany/i.test(code)) hint = "原厂限流，请降低请求频率；确认任务状态后再决定是否重新提交";
  else if (/quota|balance|payment|arrear/i.test(code)) hint = "原厂额度或账户状态异常，请联系管理员核查上游账户";
  else if (/auth|apikey|accessdenied/i.test(code)) hint = "原厂鉴权或权限异常，请管理员检查上游配置";
  else if (/invalidparam|badrequest/i.test(code)) hint = "原厂拒绝请求参数，请检查图片、尺寸及模型参数";
  else if (/content|safety|sensitive/i.test(code)) hint = "原厂内容安全检查未通过，请调整输入内容";
  // Do not reflect arbitrary provider messages (may contain keys or signed URLs).
  return qwenDiagnostic(body, "[QWEN_UPSTREAM_ERROR] " + hint + " [code=" + code + "]");
}

function qwenReconcile(body, reason) {
  return new Error(qwenDiagnostic(body, "[QWEN_USAGE_RECONCILIATION] " + reason + "; settlement requires reconciliation；用量异常待核对，不应视为免费或自动重新生成"));
}

function convertQwen(ctx) {
  const req = ctx.requestBody || {};
  const model = ctx.upstreamModel || ctx.model || req.model;
  if (!QWEN_IMAGE_MODELS.includes(model)) throw new Error("unsupported Qwen model");
  const metadata = objectValue(req.metadata, "metadata");
  if (metadata.model !== undefined && metadata.model !== model) throw new Error("can't change model with metadata");
  const mode = metadata.upstream_mode ?? "async";
  if (!["sync", "async"].includes(mode)) throw new Error("upstream_mode must be sync or async");
  const allowed = ["n", "size", "negative_prompt", "seed", "prompt_extend", "prompt_extend_mode", "enable_thinking", "watermark"];
  const parameters = {};
  for (const key of allowed) if (req[key] !== undefined) parameters[key] = req[key];
  const nativeParameters = objectValue(metadata.parameters, "metadata.parameters");
  for (const key of Object.keys(nativeParameters)) {
    if (!allowed.includes(key)) throw new Error("unsupported Qwen parameter: " + key);
    parameters[key] = nativeParameters[key];
  }
  for (const key of ["enable_interleave", "enable_sequential", "max_images", "thinking_mode", "mask", "partial_images"]) {
    if (req[key] !== undefined) throw new Error("unsupported Qwen parameter: " + key);
  }
  if (parameters.n === undefined) parameters.n = 1;
  if (!Number.isInteger(parameters.n) || parameters.n < 1 || parameters.n > 6) throw new Error("n must be an integer between 1 and 6");
  for (const key of ["prompt_extend", "enable_thinking", "watermark"]) {
    if (parameters[key] !== undefined && typeof parameters[key] !== "boolean") throw new Error(key + " must be a boolean");
  }
  if (parameters.seed !== undefined && (!Number.isInteger(parameters.seed) || parameters.seed < 0 || parameters.seed > 2147483647)) throw new Error("seed must be an integer between 0 and 2147483647");
  if (parameters.negative_prompt !== undefined && typeof parameters.negative_prompt !== "string") throw new Error("negative_prompt must be a string");
  if (parameters.prompt_extend_mode !== undefined && !["direct", "agent"].includes(parameters.prompt_extend_mode)) throw new Error("prompt_extend_mode must be direct or agent");
  const input = Object.assign({}, objectValue(metadata.input, "metadata.input"));
  for (const key of Object.keys(input)) if (key !== "messages") throw new Error("unsupported Qwen input: " + key);
  if (input.messages === undefined) {
    let images = [];
    if (req.image !== undefined) {
      if (req.images !== undefined) throw new Error("supply image or images, not both");
      images = Array.isArray(req.image) ? req.image : [req.image];
      if (!images.length) throw new Error("image must not be empty");
    } else if (req.images !== undefined) {
      if (!Array.isArray(req.images) || !req.images.length) throw new Error("images must be a non-empty array");
      images = req.images;
    }
    input.messages = [{role: "user", content: images.map(function(image) {return {image: image};}).concat([{text: req.prompt}])}];
  }
  if (!Array.isArray(input.messages) || input.messages.length !== 1 || !input.messages[0] || input.messages[0].role !== "user" || !Array.isArray(input.messages[0].content)) throw new Error("input.messages must contain one user message");
  let inputCount = 0, texts = 0;
  for (const part of input.messages[0].content) {
    if (!part || typeof part !== "object" || Array.isArray(part)) throw new Error("invalid image content");
    const keys = Object.keys(part);
    if (keys.length !== 1 || !["text", "image"].includes(keys[0])) throw new Error("content must contain one text or image field");
    if (part.text !== undefined) {
      if (typeof part.text !== "string" || !part.text.trim()) throw new Error("prompt must be a non-empty string");
      texts++;
    } else {
      validateQwenImage(part.image);
      inputCount++;
    }
  }
  if (texts !== 1) throw new Error("exactly one text prompt is required");
  if (inputCount > 3) throw new Error("at most 3 input images are supported");
  if (inputCount && parameters.prompt_extend_mode === "agent") throw new Error("agent prompt extension does not support image editing");
  let tier;
  if (parameters.size !== undefined) {
    if (parameters.size === "auto") delete parameters.size;
    else {
      if (typeof parameters.size !== "string") throw new Error("size must be a string");
      const match = /^(\d+)[x*](\d+)$/.exec(parameters.size);
      if (!match) throw new Error("size must be width*height, widthxheight or auto");
      const width = Number(match[1]), height = Number(match[2]), pixels = width * height;
      if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width <= 0 || height <= 0 || pixels < 512 * 512 || pixels > 2048 * 2048 || width / height > 8 || height / width > 8) throw new Error("size is outside Qwen pixel and aspect-ratio limits");
      parameters.size = width + "*" + height;
      tier = pixels <= 2250000 ? "1k" : "2k";
    }
  }
  return {body: {model: model, input: input, parameters: parameters}, inputCount: inputCount, tier: tier, synchronous: mode === "sync", service: mode === "sync" ? "multimodal-generation/generation" : "image-generation/generation", action: inputCount ? "image_to_image" : "text_to_image"};
}

function qwenUsage(body) {
  const usage = (body || {}).usage || {};
  const input = usage.input_image_count, output = usage.output_image_count;
  if (!Number.isInteger(input) || input < 0 || input > 3) throw qwenReconcile(body, "missing or invalid Qwen input_image_count");
  if (!Number.isInteger(output) || output < 1 || output > 6) throw qwenReconcile(body, "missing or invalid Qwen output_image_count");
  if (!["qima_input_1k", "qima_input_2k"].includes(usage.input_image_type) || !["qima_output_1k", "qima_output_2k"].includes(usage.output_image_type)) throw qwenReconcile(body, "missing or invalid Qwen image billing type");
  if (body.code || (body.output || {}).finished === false || ((body.output || {}).task_status && body.output.task_status !== "SUCCEEDED")) throw qwenReconcile(body, "response is not a confirmed success");
  if (usage.output_width !== undefined || usage.output_height !== undefined) {
    if (![usage.output_width, usage.output_height].every(function(v) {return Number.isSafeInteger(v) && v > 0;})) throw qwenReconcile(body, "invalid output dimensions");
  }
  const images = imageContent(body).filter(function(p) {return p.image;});
  if (images.length !== output) throw qwenReconcile(body, "Qwen output count disagrees with image results");
  if (!images.every(function(p) {return qwenHTTPImage(p.image);})) throw qwenReconcile(body, "invalid output image URL");
  const facts = emptyQwenUsage();
  facts[usage.input_image_type] = input;
  facts[usage.output_image_type] = output;
  return facts;
}
...
export function parseSubmitResponse(ctx, resp) {
  ...
  if (isQwen(ctx) && body.code) throw new Error(qwenUpstreamError(body));
...
export function extractUsage(ctx) {
  if (isQwen(ctx)) {
    if (ctx.usagePurpose === "billing_ratios") throw new Error("Qwen Image 3.0 requires task usage expression pricing");
    const converted = convertQwen(ctx);
    const facts = emptyQwenUsage();
    // For automatic output size reserve BOTH possible tiers. This is an upper
    // bound for arbitrary nonnegative configured rates, not an assumed 2K price.
    const tiers = converted.tier ? [converted.tier] : ["1k", "2k"];
    for (const tier of tiers) {
      facts["qima_input_" + tier] = converted.inputCount;
      facts["qima_output_" + tier] = converted.body.parameters.n;
    }
    return facts;
  }
  if (imageModel(ctx)) {
...
export function extractUsageOnSubmit(ctx, body) {
  if (isQwen(ctx)) return convertQwen(ctx).synchronous ? qwenUsage(body) : {};
...
export function extractUsageOnComplete(task, taskResult, body) {
  if (isQwen(task)) return taskResult.status === "SUCCESS" ? qwenUsage(body) : {};
...
export function parseTaskResult(ctx, body) {
  if (isQwen(ctx)) {
    if (!body || typeof body !== "object" || Array.isArray(body)) return {status: "UNKNOWN", reason: "[QWEN_QUERY_UNCERTAIN] 无效查询响应，请核查原任务，不要重新生成"};
    const state = (body.output || {}).task_status;
    if (body.code || ["FAILED", "CANCELED"].includes(state)) {
      // A query error is not evidence of task failure. Only explicit terminal
      // states may take the host's failure/refund path.
      return {status: ["FAILED", "CANCELED"].includes(state) ? "FAILURE" : "UNKNOWN", reason: qwenUpstreamError(body)};
    }
    if (!["PENDING", "RUNNING", "SUCCEEDED"].includes(state)) return {status: "UNKNOWN", reason: qwenDiagnostic(body, "[QWEN_QUERY_UNCERTAIN] 任务结果尚未确认，请核查原任务，不要重新生成")};
    if (state === "SUCCEEDED" && !imageContent(body).some(function(p) {return p.image;})) return {status: "UNKNOWN", reason: qwenDiagnostic(body, "[QWEN_QUERY_UNCERTAIN] 原厂报告成功但未返回图片，需要人工核对")};
  }
  const output = body.output || {};
...
export const native = {
  createQwenCompatible: function (ctx) {
    ...
    if (!QWEN_IMAGE_MODELS.includes(req.model)) throw new Error("unsupported Qwen model");
    if (req.stream !== undefined && req.stream !== false) throw new Error("Qwen compatible images do not support streaming");
    for (const key of ["mask", "partial_images", "input", "metadata", "parameters", "images"]) {
      if (req[key] !== undefined) throw new Error(key + " is not supported on Qwen compatible images");
    }
    ...
    req.metadata = {upstream_mode: "sync"};
    const converted = convertQwen({model: req.model, requestBody: req});
    return {kind: "submit", model: req.model, action: converted.action, requestBody: req};
  },
  qwenImageCreated: function (ctx, task) {
    const body = task.data || {};
    return {created: task.created_at, data: imageContent(body).filter(function(p) {return p.image;}).map(function(p) {return {url: p.image};}), usage: body.usage || {}};
  },
  ...
  openai_responses: {
    decodeRequest: function (ctx) {
      ...
      if (imageModel({ upstreamModel: ctx.upstreamModel, model: model })) {
        for (const key of ["n", "prompt_extend_mode", "enable_thinking", "enable_interleave", "max_images", "enable_sequential", "thinking_mode", "bbox_list", "color_palette"]) {
          if (Object.prototype.hasOwnProperty.call(req, key)) requestBody[key] = req[key];
        }
